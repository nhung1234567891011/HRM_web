...
export class CreateComponent implements OnInit {
        ...
        this.shiftWorkCreateForm = this.formBuilder.group(
            {
                ...
                EndDate: [null, [Validators.required]], // Ngày kết thúc áp dụng.
                RecurrenceType: [null],
                RecurrenceCount: [1], // Số lần lặp lại.
            ...
            },
            {
                ...
                ], // Thêm custom validator cho StartDate và EndDate
            }
        );

        this.shiftWorkCreateForm
            .get('StartDate')
            ?.valueChanges.subscribe(() => {
                this.shiftWorkCreateForm.updateValueAndValidity();
            });

        this.shiftWorkCreateForm.get('EndDate')?.valueChanges.subscribe(() => {
            this.shiftWorkCreateForm.updateValueAndValidity();
        });
        this.setDefaultRecurrenceType();
    }
    ...
    ngOnInit() {
        ...
        this.generateCalendar(currentYear, currentMonth, '1', 1);
    ...
    isWorkingDay(
        ...
        return this.scheduleData.some((shift) => {
            ...
            if (recurrenceType === '1') {
                ...
                console.log('288888888', date, isRecurring);
    ...
    updateShift(): void {
        const data = {
            ...
            startDate:
                this.shiftWorkCreateForm.value.StartDate?.toISOString().split(
                    'T'
                )[0],
            endDate: this.getEndOfDay(
                new Date(this.shiftWorkCreateForm.value.EndDate)
            ),
    ...
    setDefaultRecurrenceType(): void {
        const defaultValue = this.constant.dateFilters.dateFilters?.[0]?.value;
        if (defaultValue) {
            this.shiftWorkCreateForm
                .get('RecurrenceType')
    ...
    loadShiftDialog(): void {
        this.shiftService
            ...
            .subscribe((results) => {
                ...
                //         //         [Validators.required],
                //         //     ], // Mã ca làm việc.
                //         //     OrganizationId: [this.shiftWorkById.organizationId], // Đơn vị áp dụng phân ca.
                //         //     StartDate: [
                ...
                //         // const startDate = new Date(this.shiftWorkById.startDate); // Chuyển đổi startDate thành đối tượng Date

                //         // const month = startDate.getMonth() + 1; // Lấy tháng (0-indexed, cần +1)
                //         // const year = startDate.getFullYear(); // Lấy năm
    ...
    handleCreateShiftScheduling(): void {
        if (this.shiftWorkCreateForm.valid) {
            console.log(this.shiftWorkCreateForm.value);
            const formData = {
                shiftTableName: this.shiftWorkCreateForm.value.ShiftTableName,
                shiftCatalogId:
                ...
                startDate: this.shiftWorkCreateForm.value.StartDate,
                endDate: this.shiftWorkCreateForm.value.EndDate,
            ...
            this.shiftWorkService.create(formData).subscribe((result) => {
                this.router.navigate(['/shift-scheduling']);
                this.messageService.add({
                    severity: 'success',
                    summary: 'Thông báo',
                    detail: 'Thêm phân ca làm thành công',
                });
            });
        } else {
            console.log('valid');
